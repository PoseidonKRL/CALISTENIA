...
import { workoutPlan, XP_PER_CATEGORY, PREDEFINED_AVATARS, MOTIVATIONAL_QUOTES, PLAYER_TITLES, MISSION_XP_AMOUNTS, PREDEFINED_MISSIONS, WORKOUT_CHALLENGES } from './constants';
import type { DayPlan, Exercise, DailyMission, XpHistory, ActivityLogItem, LevelUpInfo, WorkoutChallenge } from './types';
...
const BottomNav: React.FC = () => {
    const navItems = [
        ...
        { path: '/challenges', label: 'Desafios', icon: TrophyIcon },
                ...
                {navItems.map(item => {
                    ...
                    return (
                        ...
                        >
                             {({ isActive }) => (
                                <>
                                    {isActive && <div className="absolute top-0 h-1 w-10 bg-primary rounded-b-full"></div>}
            ...
            <div className="flex items-center space-x-4 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                     <div className="flex items-center space-x-2">
                        {mission.isAutoGenerated && <SparklesIcon className="w-4 h-4 text-secondary flex-shrink-0" />}
...
const ChallengeItem: React.FC<{
    ...
    return (
        <div className={`p-4 rounded-xl border-2 transition-all duration-300 ${challenge.completed ? 'bg-accent-green/10 border-accent-green/50' : 'bg-surface border-border-color'}`}>
            <div className="flex items-center justify-between space-x-4">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-3">
                        <TrophyIcon className={`w-6 h-6 flex-shrink-0 ${challenge.completed ? 'text-accent-green' : 'text-secondary'}`} />
                        <div>
                            <p className={`font-bold text-lg ${challenge.completed ? 'text-text-secondary line-through' : 'text-text-primary'}`}>{challenge.title}</p>
                            <p className="text-sm text-text-secondary mt-1">{challenge.description}</p>
                        </div>
                    </div>
                </div>
                <div className="flex flex-col items-end ml-4 space-y-2">
                    <span className="bg-secondary/20 text-secondary text-sm font-bold px-3 py-1 rounded-full">{challenge.xp} XP</span>
                    <button
                        onClick={() => onComplete(challenge.id)}
                        disabled={challenge.completed}
                        className={`w-32 text-center py-2 px-4 text-sm font-bold rounded-lg transition-colors duration-300 flex items-center justify-center space-x-1.5 ${challenge.completed ? 'bg-accent-green/80 text-white cursor-not-allowed' : 'bg-primary text-white hover:bg-primary-hover'}`}
                    >
                        {challenge.completed ? (
                            <>
...
const ChallengesView: React.FC<{
    challenges: WorkoutChallenge[];
    onComplete: (id: string) => void;
}> = ({ challenges, onComplete }) => {
    return (
        <div className="space-y-6">
            <h2 className="text-3xl font-extrabold text-text-primary px-2">Desafios de Treino</h2>
            <p className="text-text-secondary px-2 -mt-4">Complete estes desafios únicos para ganhar grandes recompensas de XP e provar sua força.</p>
            <div className="space-y-4">
                {challenges.map(challenge => (
                    <ChallengeItem key={challenge.id} challenge={challenge} onComplete={onComplete} />
                ))}
            </div>
        </div>
    );
...
const ProfileView: React.FC<{
    ...
    const getActivityIcon = (type: ActivityLogItem['type']) => {
        switch(type) {
            case 'exercise': return <DumbbellIcon className="w-5 h-5 text-primary"/>;
            case 'mission': return <ClipboardDocumentListIcon className="w-5 h-5 text-accent-green" />;
            case 'challenge': return <TrophyIcon className="w-5 h-5 text-secondary" />;
            default: return null;
        }
    }
...
const App: React.FC = () => {
    ...
    const [lastMissionGenDate, setLastMissionGenDate] = useLocalStorage<string>('last-mission-gen-date', '');
    const [challenges, setChallenges] = useLocalStorage<WorkoutChallenge[]>(
        'workout-challenges',
        // FIX: The useLocalStorage hook expects a value as its initial state, not a lazy-initializer function.
        // The function was invoked to provide the value directly, resolving the type mismatch.
        WORKOUT_CHALLENGES.map(c => ({ ...c, completed: false }))
    );
    ...
    useEffect(() => {
        ...
        // Only run generation on a new day.
        if (today !== lastMissionGenDate) {
            const missionsToGenerate = 3;
            // Shuffle predefined missions to get a random selection
            const shuffled = [...PREDEFINED_MISSIONS].sort(() => 0.5 - Math.random());
            
            // Filter out missions that the user already has to avoid duplicates
            const existingMissionTexts = new Set(missions.map(m => m.text));
            const newMissions = shuffled
                .filter(p_mission => !existingMissionTexts.has(p_mission.text))
                .slice(0, missionsToGenerate);

            const missionObjects: DailyMission[] = newMissions.map(mission => ({
            ...
            if (missionObjects.length > 0) {
                setMissions(prev => [...prev, ...missionObjects]);
            }
            setLastMissionGenDate(today);
        }
    }, [missions, setMissions, lastMissionGenDate, setLastMissionGenDate]);
    ...
    const resetProgress = () => {
      if(window.confirm('Tem certeza de que deseja zerar todo o seu progresso? Isso inclui níveis, XP, missões e histórico.')) {
        ...
        setXpHistory([]);
        setActivityLog([]);
        setChallenges(WORKOUT_CHALLENGES.map(c => ({...c, completed: false})));
      }
    };
    ...
    const completeChallenge = (id: string) => {
        ...
        if (challenge && !challenge.completed) {
            playSound('levelUp'); // More impactful sound
            addXp(challenge.xp);
            setActivityLog(prev => [...prev, { id: `challenge-${id}`, text: challenge.title, xp: challenge.xp, type: 'challenge', completedAt: Date.now() }]);
            ...
            <div className="max-w-4xl mx-auto p-4">
                <main>
                    <Routes>
                        ...
                        <Route path="/challenges" element={<ChallengesView challenges={challenges} onComplete={completeChallenge} />} />
